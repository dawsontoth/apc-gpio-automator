...
export function handleGPIOChange(key, value, isInitial = false, triggerGroup, toggleGroup) {
    ...
    if (cfg.mode === 'switch') {
        ...
        if (isInitial || value !== cfg.lastState) {
            console.log(`GPIO Switch: Pin ${cfg.pin} on ${cfg.host} (${cfg.group}) is now ${value ? 'HIGH' : 'LOW'} -> Action: ${action}`);
        ...
        if (value) {
            ...
            if (cfg.highSince) {
                ...
                if (duration >= minTime) {
                    console.log(`GPIO Momentary: Pin ${cfg.pin} on ${cfg.host} pulse detected (${duration}ms) -> Toggling group ${cfg.group}`);
