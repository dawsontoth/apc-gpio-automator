...
export async function setGPIOOutputState(key, stateVal) {
  ...
  try {
    ...
    console.log(
      `GPIO Pin ${bcmPin} on ${output.host}:${output.port || 8888} (${output.name}) set to ${stateVal}`,
    );
    ...
    if (stateVal === 'on' && output.autoOffAfterSeconds) {
      console.log(
        `GPIO Pin ${bcmPin} on ${output.host}:${output.port || 8888} (${output.name}) will auto-off after ${output.autoOffAfterSeconds}ms`,
      );
      state.pinTimers[key] = setTimeout(() => {
        console.log(
          `Auto-off triggered for GPIO Pin ${bcmPin} on ${output.host}:${output.port || 8888} (${output.name})`,
        );
    ...
    console.error(
      `Failed to set GPIO pin ${bcmPin} on ${output.host}:${output.port || 8888} to ${stateVal}:`,
      err.message,
    );
  }
}
