...
export async function setGPIOOutputState(key, stateVal) {
    ...
    try {
        ...
        gpio.writeSync(stateVal === 'on' ? 1 : 0);
        console.log(`GPIO Pin ${pin} on ${output.host}:${output.port || 8888} (${output.name}) set to ${stateVal}`);
        ...
        if (stateVal === 'on' && output.autoOffAfter) {
            console.log(`GPIO Pin ${pin} on ${output.host}:${output.port || 8888} (${output.name}) will auto-off after ${output.autoOffAfter}ms`);
            state.pinTimers[key] = setTimeout(() => {
                console.log(`Auto-off triggered for GPIO Pin ${pin} on ${output.host}:${output.port || 8888} (${output.name})`);
                setGPIOOutputState(key, 'off');
            }, output.autoOffAfter);
        }
    } catch (err) {
        console.error(`Failed to set GPIO pin ${pin} on ${output.host}:${output.port || 8888} to ${stateVal}:`, err.message);
    }
}
