...
export async function discoverPDU(host) {
  ...
  try {
    if (state.discoveredPDUs[host]) {
      console.log(`PDU at ${host} already discovered, re-verifying...`);
    }
    ...
    if (varbinds.length > 0) {
      const baseOid = parseOid(OIDS[type].names);
      const outlets = varbinds
        .map((vb) => ({
          index: vb.oid.slice(baseOid.length).join('.'),
      ...
      if (state.discoveredPDUs[host]) {
        state.discoveredPDUs[host].session.close();
      }

      state.discoveredPDUs[host] = {
        session,
        outlets,
        type,
        sysName,
      ...
      if (state.discoveredPDUs[host]) {
        state.discoveredPDUs[host].session.close();
        delete state.discoveredPDUs[host];
        broadcastState();
      }
    }
  } catch (err) {
