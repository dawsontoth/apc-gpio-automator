...
export function getPublicState() {
    ...
    if (gpioPins.length > 0) {
        // Group gpioPins by host
        const pinsByHost = {};
        gpioPins.forEach(o => {
            if (!pinsByHost[o.host]) pinsByHost[o.host] = [];
            pinsByHost[o.host].push(o);
        });

        for (const host in pinsByHost) {
            const hostPins = pinsByHost[host];
            const pduKey = `GPIO:${host}`;
