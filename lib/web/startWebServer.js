...
import { setKasaPowerState, setKasaName } from '../kasa/setKasaState.js';
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('triggerOutlet', async ({ host, index, action }) => {
      ...
      if (host === 'Manual') {
        ...
        const delay =
          action === 'on'
            ? state.config.discoveredDeviceCustomizations?.[deviceName]?.delayOnSeconds || 0
            : state.config.discoveredDeviceCustomizations?.[deviceName]?.delayOffSeconds || 0;
      ...
      } else if (host.startsWith('Kasa')) {
        const kasaHost = host.substring(5); // e.g. "172.16.14.147"
        const kasa = state.kasaDevices[kasaHost];
        const deviceName = kasa?.name || kasaHost;
        const delay =
          action === 'on'
            ? state.config.discoveredDeviceCustomizations?.[deviceName]?.delayOnSeconds || 0
            : state.config.discoveredDeviceCustomizations?.[deviceName]?.delayOffSeconds || 0;
        ...
        if (delay > 0) {
          setTimeout(async () => {
            await setKasaPowerState(kasaHost, action);
          }, delay * 1000);
        } else {
          await setKasaPowerState(kasaHost, action);
        }
    ...
    socket.on('renameOutlet', async ({ host, index, name }) => {
      ...
      if (host.startsWith('Kasa')) {
        const kasaHost = host.substring(5);
        await setKasaName(kasaHost, name);
        return;
      }
      if (!host.startsWith('GPIO')) {
        ...
        if (state.gpioOutputs[key]) {
          ...
          const gpioCfg =
            Array.isArray(state.config.gpio) &&
            state.config.gpio.find(
              (g) =>
    ...
    socket.on('updateOutletDetails', async (updates) => {
      ...
      if (host === 'Manual') {
        ...
        if (name && name !== index) {
          if (host.startsWith('Kasa')) {
            const kasaHost = host.substring(5);
            const oldName = state.kasaDevices[kasaHost]?.name;
            await setKasaName(kasaHost, name);
            ...
            if (state.gpioOutputs[key]) {
              state.gpioOutputs[key].name = name;
              const gpioCfg =
                Array.isArray(config.gpio) &&
                config.gpio.find(
                  (g) =>
                    (g.bcmPin === index || g.pin === index) &&
                    (g.host === connKey.split(':')[0] ||
                      (!g.host && connKey.split(':')[0] === '127.0.0.1')),
                );
              if (gpioCfg) {
                gpioCfg.name = name;
                changed = true;
    ...
    socket.on('updateSchedules', (schedules) => {
      console.log('Web command: updating schedules');
      if (state.config) {
