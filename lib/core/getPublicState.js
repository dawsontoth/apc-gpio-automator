...
export function getPublicState() {
  ...
  const publicState = {
    pdus: {},
    groups: {},
    workingGroups: Array.from(state.workingGroups),
    manualDeviceStates: state.manualDeviceStates,
    groupConfigs: {},
    specialActions: [],
  };
  ...
  publicState.schedules = config.schedules || [];
  ...
  for (const host in state.discoveredPDUs) {
    ...
    pdu.outlets.forEach((o) => {
      const delayOnSeconds = config.discoveredDeviceCustomizations?.[o.name]?.delayOnSeconds;
      const delayOffSeconds = config.discoveredDeviceCustomizations?.[o.name]?.delayOffSeconds;
      allOutlets.push({
        host,
        index: o.index,
        name: o.name,
  ...
  for (const host in state.kasaDevices) {
    const kasa = state.kasaDevices[host];
    const pduKey = `Kasa:${host}`;
    ...
    const delayOnSeconds = config.discoveredDeviceCustomizations?.[kasa.name]?.delayOnSeconds;
    const delayOffSeconds = config.discoveredDeviceCustomizations?.[kasa.name]?.delayOffSeconds;
    allOutlets.push({
      host: pduKey,
      index: host,
      name: kasa.name,
  ...
  if (config.manualDevices) {
    ...
    config.manualDevices.forEach((d) => {
      allOutlets.push({
        ...
        offCommand: d.offCommand,
        delayOnSeconds: d.delayOnSeconds,
        delayOffSeconds: d.delayOffSeconds,
      });
    });
  }
